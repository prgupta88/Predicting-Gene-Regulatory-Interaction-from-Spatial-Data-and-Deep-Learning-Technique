...
def DenseNet_model(round, train_data, train_label, valid_data, valid_label, test_data, test_label):

	base_model = applications.Densenet(weights= 'imagenet',include_top=False,input_shape = train_data.shape[1:])
	...
	nb_epochs = 40
	model.fit(train_data,train_label,batch_size=4,epochs = nb_epochs,shuffle=True,verbose=1,validation_data=(valid_data, valid_label))
	...
	for i,row in enumerate(test_label):
		new_row = [row[0],row[1],row[2],test_predic[i][0]]
		predict_list.append(new_row)
	acc1, f1 = acc_and_f1(predict_list)
	print('test acc: ',acc1,'test f1: ',f1)
